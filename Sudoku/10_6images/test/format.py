...
for n in range(6):
    for size in range(22):
        for sigma in range(10):
            try:
                with open('test' + str(n) + '_' + str(size) + '_' + str(sigma) + '.jpg', 'rb') as f:
                    testData = f.read()
            except:
                print(n, size, sigma)
                continue
            ...
            sudoku.pixles[sudoku.pixles < 9.44*sudoku.pixles.mean()**0.48] = 255
            sudoku.pixles[sudoku.pixles != 255] = 0
            sudoku.doSave('result' + str(n) + '_' + str(size) + '_' + str(sigma) + '.jpg')
